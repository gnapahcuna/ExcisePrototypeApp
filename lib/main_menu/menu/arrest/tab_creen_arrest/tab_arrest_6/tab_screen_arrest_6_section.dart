...
class _TabScreenArrest6SectionState extends State<TabScreenArrest6Section>  {
  ...
  Widget _buildContent(BuildContext context) {
    ...
    return ListView.builder(
      itemCount: widget.ItemsSuspect.length,
            ...
            child: Column(
              ...
              children: <Widget>[
                  ...
                  children: <Widget>[
                    Expanded(
                      child: Container(
                        ...
                        child: Text(widget.ItemsSuspect[index].NameSuspect,
                    ...
                    Center(
                        child: InkWell(
                          onTap: () {
                            setState(() {
                              widget.ItemsSuspect[index].isCheck =
                              !widget.ItemsSuspect[index].isCheck;

                              int count = 0;
                              widget.ItemsSuspect.forEach((ev){
                                if(ev.isCheck){
                                  count++;
                                }
                              });
                              count==widget.ItemsSuspect.length?isCheckAll=true:isCheckAll=false;
                            });
                          },
                          child: Container(
                            ...
                            decoration: BoxDecoration(
                              shape: BoxShape.rectangle,
                              color:  widget.ItemsSuspect[index].isCheck
                                  ? Color(0xff3b69f3)
                                  : Colors.white,
                              border: widget.ItemsSuspect[index].isCheck
                                  ?Border.all(color: Color(0xff3b69f3),width: 2)
                                  :Border.all(color: Colors.grey[400],width: 2),
                            ),
                            child: Padding(
                                padding: const EdgeInsets.all(0.0),
                                child: widget.ItemsSuspect[index].isCheck
                                    ? Icon(
                                  Icons.check,
                                  size: 18.0,
                                  color: Colors.white,
                ...
                Padding(
                  ...
                  child: Text(
                    "จำนวนครั้งการกระทำผิด " + widget.ItemsSuspect[index].OffenseCount.toString() + " ครั้ง",
                    style: textInputStyleSub,),
                ),
                widget.ItemsSuspect[index].isCheck?Row(
  ...
  Widget _buildBottom() {
    ...
    bool isCheck = false;
    _countItem = 0;
    widget.ItemsSuspect.forEach((item) {
      if (item.isCheck)
        setState(() {
  ...
  _navigateEvidence(BuildContext context) async {
    final result = await Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => TabScreenArrest6Evidence(Title: widget.Title,Detail:widget.Detail,ItemsSuspect: widget.ItemsSuspect,)),
    );
